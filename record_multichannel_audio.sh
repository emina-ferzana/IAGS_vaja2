...
ssh pi@audio1.lmi.link "/home/pi/remotetesting/start_experiment_final.sh"
if [ $? -ne 0 ]; then
  rm -r "$1"
...
scp pi@audio1.lmi.link:/home/pi/experiment_audio_files/cache/*filt* "$1"/.
if [ $? -ne 0 ]; then
  rm -r "$1" 
...
echo "Files successfully downloaded to '$1' and remote cache directory deleted. You can now call organize_experiment_files.sh $1"
